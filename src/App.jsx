...
function PasswordGate({ onUnlock }) {
  ...
  return (
    ...
    >
      ...
      <div>
        <h2 className="handwriting" style={{ fontSize: "clamp(1.6rem,5vw,2.4rem)", color: "#c084fc", marginBottom: 8 }}>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{
          height: 1,
          background: "linear-gradient(to right, transparent, rgba(240,150,200,0.4), transparent)",
          marginBottom: 24,
        }} />

        <p
          className="handwriting"
          style={{ fontSize: "clamp(1.15rem,4.5vw,1.6rem)", color: "#6b4460", lineHeight: 2, marginBottom: 28 }}
        >
          Xuống nhà đi —
          <br />
          <motion.span
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 10, marginBottom: 28 }}>
          {["#f9a8d4", "#c084fc", "#fde68a", "#f472b6", "#ddd6fe"].map((c, i) => (
            <motion.div
              key={i}
              animate={{ y: [0, -6, 0] }}
              transition={{ repeat: Infinity, duration: 1.8 + i * 0.3, delay: i * 0.15 }}
            >
...
export default function App() {
  ...
  const [phase, setPhase] = useState("password"); // password | greeting | envelope | letter
  ...
  return (
    ...
    >
      ...
      <AnimatePresence mode="wait">
        ...
        {/* ── PASSWORD ── */}
        {phase === "password" && (
          <PasswordGate key="password" onUnlock={() => setPhase("greeting")} />
        )}
